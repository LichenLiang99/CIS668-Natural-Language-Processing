...
epatterns.append('([A-Za-z.]+)\s*@\s*([A-Za-z.]+)\.edu') #modified for dabo, added \s*
...
#orininally for aishig, horowitz, tim
#modified for bgirod,dabo,hager,hanrahan,kosecka,kunle,lam,latombe,levoy,manning,nick,ok,rinard,serafim,thm,zm 
# added \s*
ppatterns.append('\((\d{3})\)\s*(\d{3})\-(\d{4})') 
ppatterns.append('(\d{3})\s(\d{3})\s(\d{4})') #jurafsky, pal
ppatterns.append('\[(\d{3})\]\s*(\d{3})\-(\d{4})') #nass
ppatterns.append('(\d{3})\s(\d{3})\-(\d{4})') #shoham
